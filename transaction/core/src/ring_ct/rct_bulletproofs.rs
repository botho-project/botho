...
///
/// Exposing this allows implementing various offline/hardware signing
/// solutions. The idea is that the data in this object can be generated by only
...
impl SignatureRctBulletproofs {
    ...
    pub fn sign_with_committed_tags<CSPRNG: RngCore + CryptoRng, S: RingSigner + ?Sized>(
        ...
        // The caller is responsible for calling bth_cluster_tax::create_tag_signature
    ...
    pub fn verify_extended_tag_signature(
        ...
        // bth_cluster_tax::verify_tag_signature() with the deserialized data.
...
mod rct_bulletproofs_tests {
    ...
    proptest! {
        ...
        fn test_encode_decode(
            ...
            // decode(encode(&signature)) should be the identity function.
            let recovered_signature : SignatureRctBulletproofs = bth_util_serial::decode(&bytes).unwrap();
