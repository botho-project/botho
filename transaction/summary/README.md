...
A high-level `verify_tx_summary` function is provided to check the a summary against a transaction using `TxSummaryUnblindingData` and `TxOutSummaryUnblindingData` types, avaialable under the `default` or `bt-account-keys` features.
