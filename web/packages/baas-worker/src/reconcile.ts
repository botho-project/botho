 ...
 *
 * Reliable de-provisioning on cancel/payment-failure is critical: a node left
 ...
 *
 *   1. List EVERY EC2 instance tagged `botho:managed-node=true` (the same tag IAM
 *      uses to gate TerminateInstances — so the sweep can only ever see/act on
 *      managed nodes, NEVER the seed/seed2/faucet nodes).
 ...
 *      reap a paying customer's node because Stripe was briefly unreachable).
 *
...
/** Default age after which a not-yet-`running` node is considered stuck. */
...
export type ReconcileDisposition =
  ...
  | 'orphan_no_subscription_tag' // managed node with no sub tag → reaped
...
export interface ReconcileDeps {
  ...
  regions: string[]
  /** Zone for node hostnames (DNS cleanup). Default: testnet.botho.io. */
  nodeDomain?: string
  /** Age (ms) after which a non-`running` node is reaped as stuck. */
...
export async function reconcileOnce(deps: ReconcileDeps): Promise<ReconcileReport> {
  ...
  for (const region of deps.regions) {
    ...
    try {
      instances = await deps.ec2.describeManagedNodes(region)
    } catch (err) {
      // Can't list this region this cycle — skip it entirely (next cycle retries).
      console.error('reconcile: describeManagedNodes failed', region, String(err))
      continue
    }
...
/** Classify + (if needed) reap a single managed-node instance. */
async function reconcileInstance(
  inst: Ec2Instance,
  region: string,
  deps: ReconcileDeps,
  cfg: { nodeDomain: string; stuckMs: number; now: number },
  ...
  // A managed node with NO subscription tag can never map to a paying customer —
  // it is an orphan by definition (e.g. a launch that lost its tag, or a manual
  // box that wrongly carries botho:managed-node). Reap it.
  if (!subscriptionId) {
    await reap(inst, region, deps, cfg.nodeDomain)
    return {
      instanceId: inst.instanceId,
      region,
  ...
  try {
    ...
    // Transient Stripe error → SKIP (never reap a paying node on a Stripe hiccup).
...
async function reap(
  ...
  //    botho:managed-node=true resources, so this can never hit a seed/faucet node.
  try {
  ...
  // 2. Delete the DNS record if we can derive the hostname from the node-id tag.
  if (inst.nodeIdTag) {
    try {
      await deps.dns.deleteARecord(nodeHostname(inst.nodeIdTag, nodeDomain))
    } catch (err) {
      console.error('reconcile: dns delete failed', inst.nodeIdTag, String(err))
    }
  }
...
/** The managed-node tag the sweep filters on (re-exported for callers/docs). */
export const MANAGED_NODE_TAG = TAG_MANAGED_NODE
