...
- `SUBNET_ID` — the subnet the seed/faucet nodes run in (e.g. `subnet-0abc…`).
...
key-pair (`botho-nodes`) match the proven recipe and `node-config.ts`. Expand the
...
|-----|--------|------------|
| `DescribeManagedNodes` | `ec2:DescribeInstances` | `Resource: *` (AWS cannot resource-scope Describe). Read-only; the worker further filters on the `botho:managed-node` tag. |
| `RunManagedNodeConstrained` | `ec2:RunInstances` | `ec2:InstanceType == t4g.medium`, `ec2:Region == us-west-2`, **required tag** `aws:RequestTag/botho:managed-node == true`, and `aws:TagKeys` restricted to the four `botho:*` tags. |
| `RunManagedNodeSupportingResources` | `ec2:RunInstances` | Pinned to the specific **AMI / security-group / subnet / key-pair** ARNs (plus the ENIs/volumes a launch creates). Off-recipe launches are denied. |
...
| `TerminateOnlyManagedNodes` | `ec2:TerminateInstances` | **`ec2:ResourceTag/botho:managed-node == true`** — terminate is restricted to instances already carrying the managed-node tag. |

### Why the seed / seed2 / faucet nodes are safe

This is the single most important property (#458 §5):

1. **Terminate is tag-conditioned.** `TerminateOnlyManagedNodes` only permits
   `ec2:TerminateInstances` on resources where `ec2:ResourceTag/botho:managed-node`
...
2. **The tag cannot be forged onto them.** `CreateTags` is allowed **only** as
   ...
   provisioner therefore cannot add `botho:managed-node=true` to an existing
   seed/faucet node to make it terminable.
3. **The reconciliation sweep only sees managed nodes.** `describeManagedNodes`
   (the cron's only listing) filters on `tag:botho:managed-node=true`, so the
...
- the `TerminateInstances` statement carries the
  `ec2:ResourceTag/botho:managed-node == true` condition (the load-bearing
  guarantee), and
