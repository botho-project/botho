...
//! Shared harness body for the bridge federation [`AttestationSet`] threshold
//! logic (fuzz target `fuzz_bridge_attestation`).
//!
...
//!
//! This harness drives `Arbitrary`-generated sequences of `insert` /
...
//! #920 (shared-harness extraction so the native-smoke driver runs the exact
//! same body on macOS).
...
/// Decode a [`FuzzInput`] from raw bytes the same way libFuzzer would and run
/// the harness. Bytes that cannot form a `FuzzInput` are skipped (mirroring
/// libFuzzer discarding a failed `Arbitrary` construction).
pub fn run_from_bytes(data: &[u8]) {
    if let Some(input) = crate::decode_take_rest::<FuzzInput>(data) {
        run(&input);
    }
}
