...
//! Shared harness body for the federation peer-push HTTP status-line parser
//! (fuzz target `fuzz_federation_status_line`; #897, Tier B of #892).
//!
...
//! transport), #885 (bounded peer-response read), #920 (shared-harness
//! extraction so the native-smoke driver runs the exact same body on macOS).
...
/// the harness. Bytes that cannot form a `FuzzInput` are skipped.
