...
//! libFuzzer entry point for the `bth-bridge-service` mint-path byte parsers.
//! The harness body (mode enum + all semantic assertions) lives in
//! `botho_fuzz::bridge_solana_parse` so this coverage-guided target and the
...
use botho_fuzz::bridge_solana_parse::{run, Mode};
