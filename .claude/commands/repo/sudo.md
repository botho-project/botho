...
3. Remove it, then re-validate so the removal didn't break anything:
   ...
   > **This `rm` is denied under the default `guards.rmScope=repo`** (tag
   > `rm-scope-unresolved-var`), which makes `--remove` non-functional inside a
   > guarded session — and unlike the write-side guard, running with zero
   > managed worktrees does not avoid it. See the **Guard note** in step 5 for
   > the full trigger and the by-hand remedy.
...
> **Guard note — this command's writes *and* its removals are denied by
> `hooks/repo/guard-destructive.sh`, by design.** Two separate guards are
> involved, with **different triggers**. The removal half is the broader one,
> and it is the half most likely to be missed.
>
> ---
>
> **Writes — tag `worktree-write-confinement-unresolved-var`.** The
> worktree-write-confinement guard fails **closed** on any Bash write whose
> target is an unexpanded shell variable, because it cannot tell where the
> write will land. Both writes below are exactly that shape:
>
...
> from the main checkout too. With zero managed worktrees this guard fails open
> and the writes run unchanged.
>
> ---
>
> **Removals — tag `rm-scope-unresolved-var`.** Under `guards.rmScope=repo` the
> guard fails **closed** on any `rm` whose target is an unexpanded shell
> variable from the path root down. All four of this command's `rm` calls are
> that shape, and all four are denied:
>
> | Call | Where | Role |
> |---|---|---|
> | `sudo rm -f "$DROPIN"` | step 3 | documented uninstall flow |
> | `rm -f "$TMP"` | step 5 | cleanup after the candidate fails `visudo -cf` |
> | `rm -f "$TMP"` | step 5 | cleanup after a successful install |
> | `sudo rm -f "$DROPIN"` | step 5 | **post-install rollback** |
>
> **The removal trigger is config-only, and `repo` is the default.**
> `rm_scope_repo_enabled()` consults `guards.rmScope`, `LOOM_RM_SCOPE` and
> `REPO_RM_SCOPE` and nothing else — there is no worktree condition of any
> kind. The zero-managed-worktrees carve-out that makes the write denials
> avoidable does **not** apply here: a worktree-free repo — precisely where
> this document recommends running `/repo:sudo` — still hits every one of them.
>
> **Spelling the path out literally does not help either.** `sudo rm -f
> /etc/sudoers.d/<user>-nopasswd` is denied too, by rm-scope's ordinary
> out-of-repo rule rather than the unresolved-variable one. `/etc/sudoers.d/`
> is outside the repo under *any* resolution, so there is no rewrite of these
> lines that satisfies the guard while it is enabled.
>
> **Two consequences to be explicit about:**
>
> - **`--remove` (step 3) does not work** under the default configuration. That
>   is not a failure-mode path — it is the documented uninstall flow, taken
>   deliberately in normal use.
> - **The step-5 rollback does not work.** If the post-install `visudo -c`
>   fails, the `sudo rm -f "$DROPIN"` meant to undo the install is denied and
>   **the drop-in stays in `/etc/sudoers.d/` as live policy**. The candidate
>   was already validated by `visudo -cf` before the `cp`, so the file itself
>   is known-good syntax and `sudo` keeps working — but the grant is in force
>   and nothing removed it.
>
> **Operator remedy when the rollback is denied.** Treat the drop-in as
> installed and remove it by hand from a host session, using a fully literal
> path (a command substitution would be denied for the same reason):
>
> ```bash
> id -un                                     # note the name, e.g. "robb"
> sudo rm -f /etc/sudoers.d/robb-nopasswd    # literal path — substitute your name
> sudo visudo -c                             # confirm the chain is clean again
> sudo -n true                               # should now FAIL (password required)
> ```
>
> ---
>
> **Do not "fix" the writes by hardcoding a predictable temp path.** `mktemp`'s
...
>
> **What to do when any of these are denied**: stop and run the step from a
> host session — a plain shell/SSH login, or any Claude session outside a
> Loom-managed checkout — where these guards are not installed. Never route
> around them by switching tools, weakening the temp path, or disabling
> `guards.worktreeIsolation` / `REPO_GUARD_WORKTREE_ISOLATION` /
> `guards.rmScope` / `REPO_RM_SCOPE` to push the operation through; those
> escape hatches are the operator's call, not the agent's.
