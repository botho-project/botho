...
//! such as `bth_consensus_api::consensus_common::ProposeTxResult` and
//! `bth_transaction_core::validation::TransactionValidationError`.
    ...
    fn from(src: MintValidationError) -> Self {
        match src {
            MintValidationError::InvalidBlockVersion(block_version) => Self {
                code: MintValidationResultCode::InvalidBlockVersion.into(),
                block_version: *block_version,
                ..Default::default()
            },
            MintValidationError::InvalidTokenId(token_id) => Self {
                code: MintValidationResultCode::InvalidTokenId.into(),
                token_id: *token_id,
                ..Default::default()
            },
            MintValidationError::InvalidNonceLength(len) => Self {
                code: MintValidationResultCode::InvalidNonceLength.into(),
                nonce_length: len as u64,
                ..Default::default()
            },
            MintValidationError::InvalidSignerSet => Self {
                code: MintValidationResultCode::InvalidSignerSet.into(),
                ..Default::default()
            },
            MintValidationError::InvalidSignature => Self {
                code: MintValidationResultCode::InvalidSignature.into(),
                ..Default::default()
            },
            MintValidationError::TombstoneBlockExceeded => Self {
                code: MintValidationResultCode::TombstoneBlockExceeded.into(),
                ..Default::default()
            },
            MintValidationError::TombstoneBlockTooFar => Self {
                code: MintValidationResultCode::TombstoneBlockTooFar.into(),
                ..Default::default()
            },
            MintValidationError::Unknown => Self {
                code: MintValidationResultCode::Unknown.into(),
                ..Default::default()
            },
            MintValidationError::AmountExceedsMintLimit => Self {
                code: MintValidationResultCode::AmountExceedsMintLimit.into(),
                ..Default::default()
            },
            MintValidationError::NoGovernors(token_id) => Self {
                code: MintValidationResultCode::NoGovernors.into(),
                token_id: *token_id,
                ..Default::default()
            },
            MintValidationError::NonceAlreadyUsed => Self {
                code: MintValidationResultCode::NonceAlreadyUsed.into(),
                ..Default::default()
            },
            MintValidationError::NoMatchingMintConfig => Self {
                code: MintValidationResultCode::NoMatchingMintConfig.into(),
                ..Default::default()
            },
            MintValidationError::MintingToFogNotSupported => Self {
                code: MintValidationResultCode::MintingToFogNotSupported.into(),
                ..Default::default()
            },
        }
    }
...
/// Convert bth_ledger_db::ActiveMintConfig -->
/// consensus_config::ActiveMintConfig
impl From<&bth_ledger_db::ActiveMintConfig> for consensus_config::ActiveMintConfig {
    fn from(src: &bth_ledger_db::ActiveMintConfig) -> Self {
        Self {
...
/// bth_ledger_db::ActiveMintConfig
impl TryFrom<&consensus_config::ActiveMintConfig> for bth_ledger_db::ActiveMintConfig {
...
/// Convert bth_ledger_db::ActiveMintConfigs -->
/// consensus_config::ActiveMintConfigs
impl From<&bth_ledger_db::ActiveMintConfigs> for consensus_config::ActiveMintConfigs {
    fn from(src: &bth_ledger_db::ActiveMintConfigs) -> Self {
        Self {
...
/// bth_ledger_db::ActiveMintConfigs
impl TryFrom<&consensus_config::ActiveMintConfigs> for bth_ledger_db::ActiveMintConfigs {
    type Error = ConversionError;
...
mod conversion_tests {
    ...
    use bth_transaction_core::mint::MintConfig;
    use bth_transaction_core_test_utils::create_mint_config_tx_and_signers;
    ...
    fn test_convert_active_mint_config() {
        ...
        let source = bth_ledger_db::ActiveMintConfig {
        ...
        // Converting bth_ledger_db::ActiveMintConfig ->
        // consensus_config::ActiveMintConfig -> bth_ledger_db::ActiveMintConfig
        // should be the identity function.
        {
            let external = consensus_config::ActiveMintConfig::from(&source);
            let recovered = bth_ledger_db::ActiveMintConfig::try_from(&external).unwrap();
            assert_eq!(source, recovered);
        }
        ...
        {
            let bytes = encode(&source);
            let recovered = consensus_config::ActiveMintConfig::decode(bytes.as_slice()).unwrap();
        ...
        // Encoding with protobuf, decoding with prost should be the identity function.
        {
            ...
            let recovered: bth_ledger_db::ActiveMintConfig = decode(&bytes).unwrap();
            assert_eq!(source, recovered);
        }
    }
    ...
    fn test_convert_active_mint_configs() {
        ...
        let source = bth_ledger_db::ActiveMintConfigs {
            configs: vec![bth_ledger_db::ActiveMintConfig {
                mint_config: MintConfig {
                    token_id: 123,
                    signer_set,
                    mint_limit: 10000,
        ...
        // Converting bth_ledger_db::ActiveMintConfigs ->
        // consensus_config::ActiveMintConfigs -> bth_ledger_db::ActiveMintConfigs
        // should be the identity function.
        {
            let external = consensus_config::ActiveMintConfigs::from(&source);
            let recovered = bth_ledger_db::ActiveMintConfigs::try_from(&external).unwrap();
            assert_eq!(source, recovered);
        }
        ...
        {
            let bytes = encode(&source);
            let recovered = consensus_config::ActiveMintConfigs::decode(bytes.as_slice()).unwrap();
            assert_eq!(
                recovered,
        ...
        // Encoding with protobuf, decoding with prost should be the identity function.
        {
            let external = consensus_config::ActiveMintConfigs::from(&source);
            let bytes = external.encode_to_vec();
            let recovered: bth_ledger_db::ActiveMintConfigs = decode(&bytes).unwrap();
            assert_eq!(source, recovered);
        }
    }
}
