...
1. Run `scp_play`: `MC_LOG=trace cargo run -p bt-consensus-scp-play -- --scp-debug-dump /tmp/scp/4`
...
1. Extract the archive and run `scp_play` (inside `public/`): `MC_LOG=trace cargo run -p bt-consensus-scp-play -- --scp-debug-dump /tmp/node3.test.mobilecoin.com:8443/`
