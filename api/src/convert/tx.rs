...
/// Convert bth_transaction_core::tx::Tx --> external::Tx.
...
/// Convert external::Tx --> bth_transaction_core::tx::Tx.
...
mod tests {
    ...
    fn test_convert_tx() {
        ...
        for block_version in BlockVersion::iterator() {
            ...
            {
                let bytes = bth_util_serial::encode(&tx);
                let recovered_tx = bth_util_serial::decode(&bytes).unwrap();
                assert_eq!(tx, recovered_tx);
            }

            // Converting bth_transaction_core::Tx -> external::Tx -> bth_transaction_core::Tx
            ...
            // Encoding with protobuf, decoding with prost should be the identity function.
            {
                ...
                let recovered_tx: Tx = bth_util_serial::decode(&bytes).unwrap();
                assert_eq!(tx, recovered_tx);
            }
        }
    }
