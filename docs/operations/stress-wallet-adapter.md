...
Input is capped at **128 MiB**, with a bounded read of at most that limit plus one byte before rejection. Continuous testnet block production and the allowed campaign payments can exceed the previous 32-MiB history limit. The controller must still validate projected history size and combined controller/signer memory use before launch; this input ceiling is not a guarantee of resource sufficiency. The 256-KiB transaction size and fee limits are unchanged.
