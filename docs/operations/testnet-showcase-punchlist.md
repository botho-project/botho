...
The source findings below describe the pinned review baseline. These subsequent changes are proposed, not merged or deployed:

- [PR #1378](https://github.com/botho-project/botho/pull/1378), for [#1375](https://github.com/botho-project/botho/issues/1375), removes the remote-node balance fallback, exposes localized balance/history scan failures with retry, and ignores stale wallet reads. **71 relevant local tests and web-wallet typecheck passed; hosted Web CI and cargo deny passed.** Independent Loom review is pending.
- [PR #1379](https://github.com/botho-project/botho/pull/1379), for [#1376](https://github.com/botho-project/botho/issues/1376), rejects unidentified/mismatched ingresses in the remote adapter, web endpoint adoption and production Snap. **74 adapter/web tests, 25 focused Snap tests, and adapter/web/Snap typechecks passed locally**, with a freshly built WASM/Snap bundle. Restored endpoints and reconnects are checked; named loopback development chains remain supported. This does not authenticate genesis or solve same-name chain resets. Independent Loom review is pending.
- [#1377](https://github.com/botho-project/botho/issues/1377) is ready for implementation: fund the production Snap through its public API on an isolated real node, approve a normal send, verify mining/value/change/spent state, then spend again. It needs no public-testnet recovery. A mocked-node rejection test cannot close this acceptance gate.
