...
use bth_crypto_keys::{Ed25519Pair, Ed25519Signature, KeyError, SignatureError, Signer, Verifier};
...
pub enum ConsensusMsgError {
    ...
    /// Ledger db error: {0}
    LedgerDbError(bth_ledger_db::Error),
...
impl From<bth_ledger_db::Error> for ConsensusMsgError {
    fn from(src: bth_ledger_db::Error) -> Self {
        ConsensusMsgError::LedgerDbError(src)
    }
}

impl From<bth_util_serial::encode::Error> for ConsensusMsgError {
    fn from(_src: bth_util_serial::encode::Error) -> Self {
...
mod tests {
    use super::*;
    use bth_consensus_scp::{ballot::Ballot, msg::*, QuorumSet, SlotIndex};
    use bth_ledger_db::test_utils::get_mock_ledger;
    use bth_peers_test_utils::test_node_id_and_signer;
    ...
    fn test_serialization() {
        ...
        let ser = bth_util_serial::serialize(&msg.scp_msg.sender_id).unwrap();
        let m: NodeID = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.scp_msg.sender_id, m);

        let ser = bth_util_serial::serialize(&msg.scp_msg.slot_index).unwrap();
        let m: SlotIndex = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.scp_msg.slot_index, m);

        let ser = bth_util_serial::serialize(&msg.scp_msg.quorum_set).unwrap();
        let m: QuorumSet = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.scp_msg.quorum_set, m);

        let ser = bth_util_serial::serialize(&msg.scp_msg.topic).unwrap();
        let m: Topic<ConsensusValue> = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.scp_msg.topic, m);

        let ser = bth_util_serial::serialize(&msg.scp_msg).unwrap();
        let m: Msg<ConsensusValue> = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.scp_msg, m);

        let ser = bth_util_serial::serialize(&msg.prev_block_id).unwrap();
        let b: BlockID = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.prev_block_id, b);

        let ser = bth_util_serial::serialize(&msg.signature).unwrap();
        let s: Ed25519Signature = bth_util_serial::deserialize(&ser).unwrap();
        assert_eq!(msg.signature, s);

        let serialized = bth_util_serial::serialize(&msg).unwrap();
        let m: ConsensusMsg = bth_util_serial::deserialize(&serialized).unwrap();
        assert_eq!(msg, m);
    }
