...
use bth_cluster_tax::{
    capitalized_reset_charge, demurrage_charge, ClusterFactorCurve, BLOCKS_PER_YEAR_5S,
...
pub fn due(v: u64, fi: u64, fo: u64, age: u64, rate: u32) -> u64 {
    demurrage_charge(v, fi, age, rate, BLOCKS_PER_YEAR_5S).max(capitalized_reset_charge(
        v,
        fi,
        fo,
        SETTLEMENT_HORIZON_BLOCKS,
...
pub fn fee(d: &[u64], outputs: usize, bits: u32) -> Option<u64> {
    if d.is_empty() || d.len() > 16 || !(1..=16).contains(&outputs) {
        return None;
    }
    let raw = quantize(d.iter().map(|&v| v as u128).sum(), bits)
        + UNIT as u128 * d.len().max(outputs) as u128;
    raw.try_into().ok()
}
/// Public pool prefix model. The caller supplies authenticated gross issuance;
/// this is not an authorization check or a persistent ledger/reorg
/// implementation.
...
impl Origins {
    pub fn issue(
        ...
        if kind > 1 || (kind == 1) != message.is_some() {
            return Err("origin kind/message");
        }
        if message.is_some_and(|m| self.messages.contains(&m)) {
            return Err("replay");
        }
    ...
    pub fn factor(&self, tags: &[((u8, u64), u32)]) -> Result<u64, &'static str> {
        if tags.len() > 32 {
            return Err("tag count");
        }
        let mut previous = None;
        let mut sum = 0u64;
        let mut weighted = 0u64;
        for &(id, w) in tags {
            if previous.is_some_and(|p| p >= id) || w == 0 || w > 1_000_000 {
                return Err("noncanonical tag");
            }
            previous = Some(id);
            sum += w as u64;
            if sum > 1_000_000 {
                return Err("weight sum");
            }
...
fn ct1_pure_bounds_and_origin_rollback_model() {
    ...
    assert_eq!(
        capitalized_reset_charge(u64::MAX, 6000, 5999, SETTLEMENT_HORIZON_BLOCKS, u32::MAX, 1),
        0
    );
    assert_eq!(
        capitalized_reset_charge(100, 6000, 3500, u64::MAX - 1, 200, 1),
        1
    );
    for bits in 2..=4 {
        for exp in 0..68 {
            ...
            for v in [step - 1, step, step + 1] {
                if v > 16 * u64::MAX as u128 {
                    continue;
                }
                let q = quantize(v, bits);
                assert!(q >= v);
                if v > 0 {
                    assert!((q - v) * (1 << (bits - 1)) < v);
                }
            }
        }
    }
    ...
    origins
        .issue(1, 17280, 10u64.pow(12), Some([1; 32]))
        .unwrap();
    let issued = origins.clone();
    assert!(origins.issue(1, 17280, 1, Some([1; 32])).is_err());
    assert_eq!(origins, issued);
    // Model rollback restores BOTH pool state and message-consumption state.
    origins = before.clone();
    ...
    assert_eq!(
        origins.factor(&[((0, 0), 1_000_000)]),
        before.factor(&[((0, 0), 1_000_000)])
    );
    assert!(due(10u64.pow(12), 6000, 1000, 0, 200) > 0);
}

#[test]
fn ct1_self_hops_and_multi_input_rounding_are_explicit() {
    let mut value = 1_000_000_000_000_000u64;
    let mut fees = 0u64;
    for _ in 0..5 {
        let charge = due(value, 6000, 6000, 10, 200);
        let payment = fee(&[charge], 1, 2).unwrap();
        assert!(payment >= UNIT);
        value -= payment;
        fees += payment;
    }
    assert_eq!(value + fees, 1_000_000_000_000_000);
    // Preserving tags cannot reset the public origin factor. Explicit deflation
    // is charged, even if its current-age accrued term is tiny.
    assert!(due(value, 6000, 1000, 10, 200) > due(value, 6000, 6000, 10, 200));
    let amounts = [9u64; 16];
    assert_eq!(
        fee(&amounts, 1, 2),
        Some(16 * UNIT + quantize(144, 2) as u64)
    );
    assert!(fee(&[u64::MAX; 16], 16, 2).is_none());
    assert!(fee(&[], 1, 2).is_none());
    assert!(fee(&[0], 0, 2).is_none());
    let mut origins = Origins::default();
    origins.issue(0, 0, 1_000_000_000_000, None).unwrap();
    let initial = origins.factor(&[((0, 0), 1_000_000)]).unwrap();
    origins
        .issue(0, 17279, 100_000_000_000_000_000, None)
        .unwrap();
    assert!(origins.factor(&[((0, 0), 1_000_000)]).unwrap() > initial);
    let closed = origins.wealth[&(0, 0)];
    origins.issue(0, 17280, 1, None).unwrap();
    assert_eq!(origins.wealth[&(0, 0)], closed);
}
