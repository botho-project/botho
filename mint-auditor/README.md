...
```
$ ./target/docker/release/bth-mint-auditor \
      scan-ledger \
...
$ python3 integration_test.py \
    ...
    --mint-client-bin ../../target/docker/release/bth-consensus-mint-client \
...
```
    MC_LOG=bth_mint_auditor=debug cargo run -p bth-mint-auditor -- \
        scan-ledger \
        --gnosis-safe-config gnosis-safe.toml \
        --ledger-db /tmp/ledger-db \
        --mint-auditor-db /tmp/bth-auditor-db
```
