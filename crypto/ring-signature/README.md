...
* Botho amount commitments. These are Pedersen commitments to numbers which
  represent an amount of some token on the blockchain, in the smallest representable units.
* Botho ring signatures. These are MLSAG ring signatures (see Ring CT) which sign over
...
live in the `bth-transaction-core` crate, one level higher. Most of the actual blockchain
data structures are defined in `bth-blockchain-types`.
