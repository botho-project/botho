...
fn try_digestible_struct(
    ...
    let expanded = quote! {
        impl #impl_generics bth_crypto_digestible::Digestible for #ident #ty_generics #where_clause {
            fn append_to_transcript<DT: bth_crypto_digestible::DigestTranscript>(&self, context: &'static [u8], transcript: &mut DT) {
    ...
    let expanded = if let Some(field_ident) = &fields[0].ident {
        quote! {
            impl #impl_generics bth_crypto_digestible::Digestible for #ident #ty_generics #where_clause {
                ...
                fn append_to_transcript_allow_omit<DT: bth_crypto_digestible::DigestTranscript>(&self, context: &'static [u8], transcript: &mut DT) {
