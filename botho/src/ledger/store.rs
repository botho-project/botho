...
impl Ledger {
    ...
    fn add_block_inner(
        ...
        let block_fees = validation::ValidationReads::validate_ordinary_block(
            self,
            block,
            &state,
            validation::RootRule::Legacy,
        )?;
    ...
    pub fn verify_transaction(&self, tx: &BothoTransaction) -> Result<(), LedgerError> {
        validation::ValidationReads::verify_transaction(self, tx)
    }
    ...
    fn verify_cluster_tag_inheritance(&self, tx: &BothoTransaction) -> Result<(), LedgerError> {
        validation::ValidationReads::verify_cluster_tag_inheritance(self, tx)
    }
    ...
    fn verify_settlement(&self, tx: &BothoTransaction) -> Result<(), LedgerError> {
        validation::ValidationReads::verify_settlement(self, tx)
    }
    ...
    fn verify_consensus_fee_floor(
        ...
        validation::ValidationReads::verify_consensus_fee_floor(self, tx, block_height)
    }
    ...
    pub fn consensus_fee_floor(
        ...
        validation::ValidationReads::consensus_fee_floor(self, tx, block_height)
    }
    ...
    fn effective_cluster_wealth_from_outputs(
        ...
        validation::ValidationReads::effective_cluster_wealth_from_outputs(self, outputs)
    }
    ...
    fn import_floor_factor_from_outputs(&self, outputs: &[TxOutput]) -> Result<u64, LedgerError> {
        validation::ValidationReads::import_floor_factor_from_outputs(self, outputs)
    }
    ...
    pub fn ring_centroid_floored_factor(
        ...
        validation::ValidationReads::ring_centroid_floored_factor(
            self,
            claimed_factor,
            ring_members,
            curve,
    ...
    pub fn get_lottery_validation_candidates(
        ...
        let txn = self
            ...
            .read_txn()
            .map_err(|e| LedgerError::Database(format!("Failed to start read txn: {}", e)))?;
        validation::lottery_candidates(
            &self.write_tables(),
            &txn,
            block_height,
            prev_block_hash,
            config,
            validation::ReadPolicy::Legacy,
            &mut |_| Ok(()),
        )
    }
...
mod tests {
    ...
    /// accepted-chain/signature rehearsal.
    #[test]
    fn ct1_research_ledger_decoy_adapter_parity() {
        ...
        let candidates: Vec<_> = utxos
            .iter()
            .filter(|u| u.created_at <= 90 && !excluded.contains(&u.output.target_key))
            .map(|u| OutputCandidate::from_utxo(u, 100))
            .collect();
        let direct = GammaDecoySelector::new()
            ...
            .unwrap();
        assert_eq!(
            selected.iter().map(|o| o.target_key).collect::<Vec<_>>(),
            direct.iter().map(|o| o.target_key).collect::<Vec<_>>()
        );
        assert!(selected.iter().all(|o| !excluded.contains(&o.target_key)));
        let immature = utxos.iter().find(|u| u.created_at == 91).unwrap();
        assert!(selected
            .iter()
            .all(|o| o.target_key != immature.output.target_key));
        assert!(ledger
            .get_decoy_outputs_for_input(
                24,
                &excluded,
