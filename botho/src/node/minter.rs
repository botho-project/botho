...
fn mint_loop(
    ...
    // Hybrid ML-KEM ciphertext for the reward output, encapsulated to the
    // minter's own published KEM key (regenerated alongside the stealth keys).
    let mut cached_kem_ciphertext: Option<Vec<u8>> = None;
    ...
    while !shutdown.load(Ordering::Relaxed) {
        ...
        if current_version != last_work_version || cached_work.is_none() {
            ...
            // Generate new stealth keys for this work unit. The coinbase has no
            // external sender, so the minter encapsulates an ML-KEM shared
            // secret against its OWN published KEM key and folds it into the
            // one-time target key (hybrid stealth envelope, whitepaper §4.2,
            // #958). This lets the minter later scan and spend its reward.
            // A KEM-less (pre-v2 / classical) address falls back to a
            // classical stealth output with no ciphertext.
            let (target_key, public_key, kem_ciphertext) =
                MintingTx::coinbase_stealth_fields(&address);
            cached_target_key = target_key;
            cached_public_key = public_key;
            cached_kem_ciphertext = kem_ciphertext;
        ...
        if hash_value < work.difficulty {
            ...
            let minting_tx = MintingTx {
                ...
                kem_ciphertext: cached_kem_ciphertext.clone(),
