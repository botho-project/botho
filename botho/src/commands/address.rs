...
pub fn run(config_path: &Path, save_path: Option<&str>) -> Result<()> {
    ...
    if let Some(path) = save_path {
        ...
        if path.ends_with(".pq") {
            let addr = Address::Quantum(wallet.quantum_safe_address());
        ...
        let addr = Address::Classical(wallet.default_address());
        addr.save_to_file(path)?;
    ...
    let classical_addr = format_classical_address(&wallet.default_address());
    ...
    #[cfg(feature = "pq")]
    {
        ...
        let quantum_addr = format_quantum_address(&wallet.quantum_safe_address());
