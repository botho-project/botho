...
\emph{signatures from a trusted signer set}, not by on-chain proof of the
counterparty chain (ADR~0002). The normative decision is:
...
federation signatures, domain-separated and bound to the exact order. The
...
\begin{itemize}
  \item \textbf{\BTH\ reserve release} is authorized by a $t$-of-$n$ threshold
    of the validators' \textbf{Ed25519} node keys, reusing the operator-signed
    action envelope machinery (domain-separated payloads, reserve-then-apply
    nonce replay protection, audit log).
  \item \textbf{Solana wBTH mint} is authorized by the same validators'
  ...
  \item \textbf{Ethereum wBTH mint} requires \textbf{secp256k1}, which SCP node
    ...
    and the Ethereum mint authority is a \textbf{Gnosis Safe} whose owners are
    those secp256k1 keys and which holds \texttt{MINTER\_ROLE} on the wBTH
    contract.
\end{itemize}

\subsubsection{Attestation envelopes and exactly-once semantics}

Every authorization is carried in a \emph{domain-separated, order-bound,
...
\begin{enumerate}
  \item \textbf{Exactly-once mint} --- one confirmed \BTH\ deposit mints wBTH at
    most once. Each order carries a deterministic 32-byte on-chain
    \texttt{orderId}; the destination contract records it and reverts on a
    duplicate.
  \item \textbf{Exactly-once release} --- one confirmed wBTH burn releases
...
On Ethereum the contract's privileged roles are split across \emph{three
...
\end{tabular}
\end{center}
The deployer receives no roles. The $t$-of-$n$ enforcement lives \emph{in the
validator Safe}, not in the token contract: the contract only checks that the
caller holds \texttt{MINTER\_ROLE}. A single relayer EOA submits
...
refuses to operate if the on-chain mint authority equals the local relayer key
(a lone-hot-key configuration).
...
vector, so the bridge reads its factor directly and mints only for factor-1
deposits (a non-factor-1 deposit is rejected with an audit event and never
mints). Reciprocally, releases spend \emph{only} factor-1 / background reserve
outputs, so a release can never launder cluster provenance and the reserve
change stays zero-demurrage. Figure~\ref{fig:bridge:mint} shows the wrap flow.
...
recipients receive to one-time stealth addresses, and amounts are (on the
confidential-amounts roadmap) hidden in Pedersen commitments. wBTH is fully
transparent. The bridge is therefore a designed privacy boundary with explicit
leakage semantics (ADR~0004).
...
\begin{enumerate}
  ...
  \item \textbf{The wrapped side is public.} Anyone holding or moving wBTH is
    linkable on the host chain. Wallet UX must warn that \emph{wrapping exits
    the anonymity set}.
  \item \textbf{Unwrap re-shields the recipient.} A burn $\to$ release pays out
    to a \emph{fresh one-time stealth address}, never a reused address, breaking
    the on-\Botho\ link to the destination-chain burn.
...
\begin{quote}
\textbf{Unwrapping mints \BTH\ into a bridge-import cluster keyed to the
...
implementation releases a factor-1 output (register row IMP-1).}
...
\begin{enumerate}
  \item \textbf{Exactly-once mint} --- one confirmed deposit mints at most once
    (order-id idempotency, on-chain replay guard, crash-safe transitions).
  \item \textbf{Exactly-once release} --- one confirmed burn releases at most
    once (record-before-broadcast, idempotent re-broadcast).
  \item \textbf{Threshold authorization} --- no mint or release without a
    $t$-of-$n$ domain-separated, order-bound signature set; an equivocating
    signer counts once and is additionally flagged by a dedicated audit event.
  \item \textbf{Peg solvency} --- \eqref{eq:bridge:peg} holds within tolerance
    (default $0$), and any drift trips a fail-closed circuit breaker.
  \item \textbf{Finality safety} --- an action fires only against a block final
    on its chain (SCP finality on \BTH; a confirmation-depth window plus
...
reserve-balance shortfall (custody theft) trips the breaker and emits a drift
alert. A chain whose live supply / reserve-balance transport is unavailable is
reported \emph{unverified} and excluded from drift math---never silently counted
as healthy.
...
\paragraph{Mainnet gate.} No bridge value moves on mainnet until an external
...
\begin{center}
\small
\begin{tabular}{p{2.1cm}p{3.2cm}p{3.2cm}p{1.5cm}p{2.0cm}}
\toprule
\textbf{Component} & \textbf{Live behavior} & \textbf{Target (this spec)} & \textbf{Status} & \textbf{Tracking} \\
\midrule
...
\#938 assigns the epoch import cluster on the release path, every present-tense
statement in \S\ref{sec:bridge:import} is target-state as recorded above.
