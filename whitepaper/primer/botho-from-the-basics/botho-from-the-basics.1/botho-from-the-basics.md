...
not in the whole universe as a zero-knowledge system like Zcash's shielded
pool would allow. Botho accepts this trade deliberately — ring signatures
...
The classical alternative — BFT voting protocols — gives *deterministic*
finality (once committed, committed forever) but traditionally requires a
fixed, globally known list of validators, which is the opposite of
permissionless.
...
- **Proof-of-work decides who gets to propose the next block — and who
  earns the new coins in it.** Miners race to find a block-header nonce
  meeting the difficulty target. Winning this race is a *lottery ticket
  weighted by computation*: it buys you the right to put a candidate block
...
for Monero. RandomX is deliberately hostile to ASICs: it executes randomly
...
roughly fair odds per watt — rather than collapsing into a specialized-
...
- **Phase 1 (about five years):** rewards start at 50 BTH per block and
  halve each year — 50, 25, 12.5, 6.25, 3.125. Summed up, this
  distributes roughly **611 million BTH**, the overwhelming majority of
  supply, across the first five years. Compare Bitcoin, which stretches
...
100,000 BTH sit mid-scale (3.5×); coins from million-BTH lineages saturate
near 6×. (The exact curve is a log-domain sigmoid — see the whitepaper;
...
- **Demurrage — the holding-cost term.** Transaction fees only bite when
  ...
  concentrated wealth. Factor-1 coins are exempt: ordinary users never pay
  it. (This is a pointed refinement of the century-old "demurrage
...
transaction-ordering games (MEV). In Botho, **miners never receive fees**
...
3. **Cluster-anchored mechanisms don't degrade under attack — they
   ...
   marginally *improves*. Attacking the mechanism funds it.
...
modest lineages: cluster factor 1×. The wallet computes the fee — base
rate × size × factor 1, no demurrage term (factor-1 coins are exempt) — a
few tens of nano-BTH. Had Amara been spending coins descended from a
...
**What an outside observer saw:** a transaction spending two-of-forty
possible inputs, to two never-before-seen output keys, in unknown amounts,
from an unknown network origin, finalized in seconds — plus a fee whose
size reveals only the *lineage class* of the coins moved. That's the whole
...
| Comparisons: Monero, MobileCoin, Zcash, TEE approaches | §2 Related Work |
