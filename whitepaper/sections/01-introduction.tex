...
of Bitcoin~\cite{nakamoto2008bitcoin} and subsequent cryptocurrencies
...
financial histories~\cite{chainanalysis}. Chain analysis companies have built entire industries
...
The Universal Declaration of Human Rights~\cite{udhr} recognizes privacy as a
fundamental right.
...
render many of these protections obsolete~\cite{shor,grover}. The ``harvest now, decrypt
...
Traditional fee markets create perverse incentives. When fees flow directly
to block producers, pressure builds toward mining centralization---larger
operations achieve lower marginal costs through economies of scale. Fee
volatility during network congestion disproportionately harms small
transactions and users in developing economies.
...
\begin{itemize}
  ...
  \item \textbf{Community consensus}: The Stellar Consensus Protocol~\cite{scp}
...
\begin{enumerate}
  \item \textbf{Hybrid post-quantum architecture}: We introduce a principled
    ...
    indefinitely, are protected by \MLKEM-768~\cite{fips203,kyber}. Sender privacy, which has
    ephemeral value, uses efficient \CLSAG ring signatures~\cite{clsag}. This achieves
  ...
  \item \textbf{Dynamic block timing}: Block intervals adapt to network
    load (3--40 seconds, 5-second steady-state reference), creating
    natural inflation dampening. Low
