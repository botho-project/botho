...
\paragraph{Confidential-amount implementation status.}
The confidential-amount target remains under implementation. Production CLSAG
transactions still publish amounts; the inactive single-input integer/R1CS
experiments in issues \#1284/\#1288 do not supply a complete confidential
transaction. They exercise exact staged floors, saturation and statement binding,
but not the final multi-input commitment linkage, conservation, fee quantization,
wire format or client migration. ADR0009 remains Proposed. The candidate contract
in \texttt{docs/design/ct-transaction-contract.md} preserves ratified EpochOrigin
while identifying remaining circulation-policy and fee-bucket decisions.
Its arithmetic vectors and source inventory are specification evidence, not
activation, an audited protocol or a mainnet-readiness claim. Timings below must
not be interpreted as measurements of that unimplemented combined CT proof.
...
\midrule
1 & 6.3M & 30 GB & 6 GB \\
5 & 31.5M & 150 GB & 24 GB \\
10 & 63M & 300 GB & 40 GB \\
20 & 126M & 600 GB & 70 GB \\
\bottomrule
\end{tabular}
\end{table}

\textit{Assumptions: the 5\,s monetary reference (6{,}307{,}200 blocks/year;
the maximum block rate and thus the storage upper bound), matching the
emission schedule of Section~\ref{sec:monetary}. Under the dynamic schedule
the realized average is slower, so these are conservative ceilings. Also
assumes 50\% block utilization and a 4 KB average transaction size.}
...
Transaction Summary:
  Amount:      50.00 BTH
  Base fee:     4,800 pico-BTH   (4 KB x 1 pico/byte x 1.2 factor)
  Cluster factor: 1.2x
  Total fee:    4,800 pico-BTH   (= 4.8e-9 BTH)
...
\midrule
Block time & 3--40s (dynamic) & 3--40s (dynamic) \\
Initial block reward & 50 BTH & 50 BTH \\
Halving interval & 120{,}000 blocks & 6{,}307{,}200 blocks \\
Ring size & 20 & 20 \\
Minimum fee & 1 pico-BTH/byte & 1 pico-BTH/byte \\
Network port & 19732 & 9732 \\
Magic bytes & \texttt{0x00075E57} & \texttt{0x000B07B0} \\
\bottomrule
\end{tabular}
\end{table}
...
Node operation is documented separately in the operator guide shipped with
the reference implementation, covering configuration
(\texttt{config.toml}), hardware sizing by role (light / full / minting),
process supervision, Prometheus metrics, and backup/restore. These are
operational concerns rather than protocol specification and are out of scope
here.
