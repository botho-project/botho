...
The effective anonymity set depends on decoy selection quality. The reduction
factors below are consistent with empirical traceability studies of
ring-signature systems, notably Möser et al.'s analysis of Monero, which
found that temporal and zero-decoy heuristics can sharply cut the effective
ring size~\cite{monerotracing}:
...
PoW preimage binding & 256-bit & 128-bit (Grover) & Minting attribution \\
\bottomrule
\end{tabular}
\end{table}
...
Eclipse attacks monopolize a victim's peer connections to control its view of
the network~\cite{eclipseattack}. \textbf{Mitigation}:
\begin{itemize}
...
\Botho's hybrid consensus changes the selfish-mining dynamics first analyzed
for longest-chain PoW~\cite{selfishmining}:
...
This is the zero-decoy / poisoned-output attack characterized empirically
against Monero~\cite{monerotracing}.
...
\textbf{Classic attack}~\cite{floodloot} (against pure PoW):
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item Modify transaction ordering/selection to change the randomness seed
  \item Bias the lottery toward controlled UTXOs
\end{enumerate}

\textbf{Defense mechanism}: The lottery seed is derived only from the
\emph{committed previous block}:
\[
\text{seed} = \Hash(\text{``LOTTERY\_SEED\_V1''} \| \text{prev\_block\_hash} \| h).
\]

\textbf{Analysis}:
\begin{itemize}
  \item The seed does not depend on the current block's transactions, merkle
    root, or ordering, so the proposer of block $h$ cannot bias the draw by
    selecting or reordering its own transactions.
  \item \texttt{prev\_block\_hash} is already externalized by SCP before block
    $h$ is built, so it is fixed for every honest and Byzantine proposer alike.
  \item The only remaining lever is to regrind the \emph{previous} block's PoW
    to change its hash --- but this costs a full PoW solution, and the payout
    it can redirect is bounded by the per-block cap (one block reward).
\end{itemize}

\textbf{Cost-benefit (the load-bearing bound)}: The pool pays out at most one
block reward per block. Regrinding the previous block to reshape the seed
therefore trades a full PoW solution (worth one block reward in expectation)
for the chance to redirect at most a fraction of one reward:
\[
\text{grinding\_cost} \;\approx\; R(h) \;>\; \Delta_{\text{payout}} \leq R(h) \times P(\text{controlled UTXO wins}).
\]

Grinding is unprofitable by construction: the cost of the extra PoW solution
exceeds the capped payout it can capture.
