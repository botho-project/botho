...
The protocol uses no post-quantum signature scheme: spend authorization
is classical (Section~\ref{sec:clsag}) and minting attribution is bound
by the proof-of-work preimage itself
(Section~\ref{sec:minting-attribution}), which is hash-based and
therefore quantum-resistant without a signature. Should a post-quantum
authorization path be introduced in a future protocol version, ML-DSA
(Module-Lattice-based Digital Signature
Algorithm)~\cite{fips204,dilithium} at security level 3 (ML-DSA-65) is
the designated signature family.
