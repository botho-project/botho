...
\midrule
Minting & 0 & 1 & PoW binding (none) & $\sim$0.2 KB & Block rewards \\
...
\input{figures/transaction-anatomy}
...
MintingTx {
    block_height: u64,
    reward: u64,                 // Public amount (picocredits)
    minter_view_key: [u8; 32],   // Bound by the PoW preimage
    minter_spend_key: [u8; 32],  // Bound by the PoW preimage
    target_key: [u8; 32],        // Stealth one-time reward key
    public_key: [u8; 32],        // Ephemeral key for minter scanning
...
\begin{itemize}
  ...
  \item Public reward amount (for supply auditability)
  \item Known sender (the minter's address keys are in the clear and
    bound by the PoW preimage; no signature is carried ---
    Section~\ref{sec:minting-attribution})
  \item The reward recipient is still hidden via a stealth output ---
    even a minter who wins repeatedly produces unlinkable reward outputs
\end{itemize}
...
\input{figures/cluster-inheritance}
...
Each minting transaction creates a new cluster. The minting
transaction's hash --- which commits to the minter's public keys, the
block height, and the PoW solution --- serves as the cluster
identifier (truncated to 64 bits):

\begin{equation}
c_{\text{new}} = \text{Trunc}_{64}\bigl(\Hash(\text{minting tx})\bigr)
\end{equation}
...
where $T_{\min} = 720$ blocks ($\approx$ 1 hour at the 5\,s reference block
time, and correspondingly longer when the network runs slower).

\textbf{Wash Trading Resistance}: Since new outputs must wait $T_{\min}$
before decay applies again, the binding constraint is age, not transaction
count: at the 5\,s reference a holder can trigger at most one decay event per
hour (24/day), so rapid self-transfers achieve no decay.

\begin{table}[h]
\centering
\caption{Tag decay limits (at the 5\,s reference; $\times$0.95 per event, one
event per 720 blocks)}
\label{tab:decay-limits}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Attack} & \textbf{Decay Events} & \textbf{Maximum Decay} \\
\midrule
Rapid wash (1 minute) & 0 & 0\% \\
Patient wash (1 day) & 24 & 71\% \\
Patient wash (1 week) & 168 & $\approx$100\% \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Cluster Wealth Accumulation}
\label{sec:cluster-wealth}

Although each minting event creates a fresh, per-block cluster id
(Equation, Section~\ref{sec:cluster-tags}), a cluster's \emph{wealth} is not
one block reward: it is the cumulative tagged value that lineage controls
across the entire UTXO set. Formally, the wealth of cluster $c$ is
\begin{equation}
W_c = \sum_{U \in \mathcal{U}} v(U)\, w_c(U),
\end{equation}
the value-weighted sum of every UTXO's attribution to $c$. As coins minted in
that lineage circulate and blend, $W_c$ accumulates through tag inheritance
and can grow to hundreds of thousands of BTH --- far above a single 50 BTH
reward. The ledger tracks $W_c$ as a cumulative \texttt{u128} per cluster.

\subsubsection{Cluster Factor Calculation}

A UTXO's scalar cluster factor is obtained by (i) collapsing its tag vector
to an \emph{effective wealth}, the tag-weighted sum of the constituent
clusters' wealths,
\begin{equation}
W_{\text{eff}}(U) = \sum_{c} w_c(U)\, W_c,
\end{equation}
and (ii) mapping that wealth through a \emph{log-domain} sigmoid that rises
across orders of magnitude of wealth:
\begin{equation}
\phi(U) = 1 + 5\,\sigma\!\left(\frac{\log_2 W_{\text{eff}}(U) - \log_2 W_{\text{mid}}}{\omega}\right),
\end{equation}
where $\sigma$ is the logistic function, $W_{\text{mid}} = 100{,}000$ BTH is
the midpoint (a cluster at the midpoint sits at exactly $3.5\times$), and
$\omega = 2.0$ octaves per sigmoid unit sets the width. The factor is clamped
to $[1\times, 6\times]$. Diffuse, low-wealth attribution dilutes
$W_{\text{eff}}$ toward zero, so genuinely small holders sit at $1\times$.

\begin{table}[h]
\centering
\caption{Progressive fee multipliers (log-domain curve, $W_{\text{mid}}=100$k BTH)}
\label{tab:fee-multipliers}
\begin{tabular}{@{}lc@{}}
\toprule
\textbf{Effective Cluster Wealth} & \textbf{Fee Multiplier} \\
\midrule
$\leq$1{,}000 BTH & $\approx$1.0$\times$ \\
$\sim$100{,}000 BTH (midpoint) & 3.5$\times$ \\
$\gtrsim$1{,}000{,}000 BTH & $\approx$5.2$\times$ -- 6.0$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\input{figures/progressive-fee}
...
\begin{itemize}
  ...
  \item \textbf{Wash trading}: Age-based gating limits decay to $\sim$24
    events per day maximum (at the 5\,s reference; fewer when blocks are
    slower).
