...
If the network has quorum intersection and the Byzantine threshold is not
exceeded---so that the intersection of any two quorums contains at least one
honest node---then SCP provides safety: no two honest nodes externalize
different values for the same slot. Safety does not require the absence of
Byzantine nodes, only that no two quorums can be satisfied entirely by
disjoint, adversary-controlled sets.
\end{theorem}

Safety and liveness are distinct properties with distinct assumptions.
Safety, as stated above, holds under quorum intersection among honest nodes
even under asynchrony. Liveness---that consensus eventually makes
progress---is a separate guarantee requiring an intact, available honest
quorum and network synchrony; it is treated in
Section~\ref{sec:consensus-liveness}. When these liveness assumptions fail
(e.g., a network partition), \Botho preserves safety by halting rather than
forking.
...
\begin{enumerate}
  ...
  \item Miner searches for a nonce whose proof-of-work hash
    $h_{\text{PoW}}$ (the RandomX preimage over the minter's address keys
    defined in Section~\ref{sec:minting-attribution}) meets the
    difficulty target, i.e. $h_{\text{PoW}} < \text{target}$
...
\midrule
Block proposal (PoW) & Variable (3--40s target, 5s reference) \\
Nomination & $\sim$1s \\
...
We compare against Nakamoto longest-chain consensus~\cite{nakamoto2008bitcoin}
and BFT-style deterministic finality as exemplified by
