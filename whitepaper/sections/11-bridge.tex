...
% Source of truth: bridge ADRs 0002-0005, 0007 (whitepaper/bridge-spec/botho-bridge-spec/refs/)
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig3-federation-custody.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig1-wrap-mint-flow.png}
...
\subsubsection{Epoch-keyed import cluster}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig2-unwrap-import-flow.png}
...
$\mathrm{import\_factor}(m) \geq F$. \emph{Live} as of protocol~5.0.0: the
implementation tags the released output to the epoch import cluster at the
elevated factor ($F = 1.5\times$ floor, $K = 1$~day epoch; register row IMP-2).}
...
IMP-2 &
  ...
  Unwrap tags the released output $100\%$ to $c_{\mathrm{import}}(m)$ at $\mathrm{import\_factor}(m)\geq F$ ($F=1.5\times$, $K=1$~day); \eqref{eq:bridge:epoch}--\eqref{eq:bridge:importfactor} (\texttt{bth\_scan.rs} assigns the epoch import cluster) &
  (same) &
  live &
  botho\#938 / \#942 \\
IMP-3 &
...
\paragraph{Import-tagging landed (protocol 5.0.0).} The import-factor machinery
of \S\ref{sec:bridge:import} is live as of protocol~5.0.0 (\#942, register row
\textbf{IMP-2}). The production unwrap path
(\texttt{bridge/service/src/bth\_scan.rs}) now tags the released recipient
output $100\%$ to the block-epoch import cluster
$c_{\mathrm{import}}(\lfloor h/K \rfloor)$ via
\texttt{import\_cluster\_id\_for\_height}, at
$\mathrm{import\_factor}(m) = \max(F,\,\mathrm{ClusterFactorCurve}(\cdot))$ with
$F = 1.5\times$ and $K = 17{,}280$ blocks; the pre-5.0.0 behavior (a factor-1
output carrying empty cluster tags) no longer applies. Every present-tense
statement in \S\ref{sec:bridge:import} therefore holds of the implementation
today.
