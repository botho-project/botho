...
$\dhsecret = rC = aR$ (as in classical CryptoNote, where $r$ is the sender's
...
\[
  s = \Hs\bigl(\dhsecret \,\|\, K \,\|\, \text{output\_index}\bigr).
\]
...
\begin{enumerate}
  ...
  \item Compute classical DH secret: $\dhsecret = rC$
  \item Encapsulate: $(c, K) \leftarrow \MLKEM.\Encap(\pk_{\text{kem}})$
  \item Compute hybrid scalar: $s = \Hs(\dhsecret \| K \| \text{output\_index})$
...
\begin{enumerate}
  ...
  \item For each output with transaction public key $R$ and ciphertext $c$:
    \begin{enumerate}
      \item Recompute classical DH secret: $\dhsecret = aR$ (\,$= rC$\,)
...
\begin{proof}
Because the stealth scalar $s = \Hs(\dhsecret \| K \| \text{output\_index})$ is a
...
\emph{either} $\dhsecret$ or $K$ is unknown to the adversary. It therefore suffices
...
unlinkability argument~\cite{cryptonote} applied to the $\dhsecret$ input.
...
non-negligible advantage $\epsilon$, given $\dhsecret$ (i.e.\ even after ECDH is
...
$\Adv'$ computes $s^* = \Hs(\dhsecret \| K_b \| \text{index})$ and
